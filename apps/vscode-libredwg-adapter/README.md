# 2D CAD Viewer — Legacy Engine Test Package
...
product and the main 2D CAD Viewer does not declare it as a dependency.

Current releases publish only **2D CAD Viewer for VS Code** to Marketplace. The
...
- the 2D CAD Viewer adapter source and build scripts used for the executable;
...
The qualification package remains separate from the MPL-2.0 2D CAD Viewer VSIX.
